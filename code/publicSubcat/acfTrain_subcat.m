function detector = acfTrain_subcat( varargin )
% Train aggregate channel features object detector.

% Eshed included the following modifications to the original code:
% 1) Commented opts.pLoad.squarify{2}=opts.modelDs(2)/opts.modelDs(1); (line 222)
% 2) Commented the assert call on line 300 %assert(all(abs(bbs(:,3)./bbs(:,4)-r)<1e-5))
% 3) Modified handling ignore boxes as well. (line 306). 
% 4) Box exclusion at .4 threshold was added.
...
if( positive ), bbs=gt; bbs=bbs(bbs(:,5)==0,:); else
  ...
  if( ~isempty(gt) )
    ...
    for i=1:n
        overlap = compOas_subcat(bbs(i,:),gt,gt(:,5));
        keep(i) = all(overlap(gt(:,5)==0)<.1) & all(overlap(gt(:,5)==1)<.4);
    end
    bbs=bbs(keep,:);
  end
end
...
function oa = compOas_subcat( dt, gt, ig )
...
for i=1:m
  for j=1:n
    ...
    t=w*h; if(ig(j)), u=min(da(i),ga(j)); else u=da(i)+ga(j)-t; end; oa(i,j)=t/u;
  end
end
end
